...
#define kdq_int_t kdq_t(int)
    ...
    odgi::graph_t smooth_abPOA(const xg::XG &graph,
        ...
        // FIXME
        /*
         * We should be able to use the already defined table in <deps/abPOA/src/seq.h>
...
/usr/bin/ld: CMakeFiles/smoothxg_objs.dir/src/smooth.cpp.o:/home/heumos/git/smoothxg/deps/abPOA/src/seq.h:30: multiple definition of `com_nst_nt4_table'; CMakeFiles/smoothxg.dir/src/main.cpp.o:/home/heumos/git/smoothxg/deps/abPOA/src/seq.h:30: first defined here
         */
        ...
        for (int i = 0; i < n_seqs; ++i) {
            ...
            for (int j = 0; j < seq_lens[i]; ++j) {
                bseqs[i][j] = nst_nt4_table[(int) seqs_[i][j]]; // TODO we make a c_str for every char in the string
            }
        }
        uint8_t **cons_seq;
        ...
        */
        ...
        paryfor::parallel_for<uint64_t>(
                ...
                [&](uint64_t block_id, int tid) {
                    ...
                    //std::cerr << std::endl;
                    //std::cerr << "After block graph. Exiting for now....." << std::endl;
                    //exit(0);
        ...
        std::cerr << "[smoothxg::smooth_and_lace] adding graph " << j++ << "/" << block_graphs.size() << " 100.000%"
                  << std::endl;
        ...
        for (uint64_t i = 0; i < path_mapping.size(); ++i) {
            {
                std::cerr << "[smoothxg::smooth_and_lace] embedding path fragment " << i << "/" << path_mapping.size()
                          << "\r";
            }
        ...
        std::cerr << "[smoothxg::smooth_and_lace] embedding path fragment " << path_mapping.size() << "/"
                  << path_mapping.size() << std::endl;
        ...
        smoothed.for_each_path_handle(
                [&](const path_handle_t &path) {
                    ...
                    if (orig_seq != smoothed_seq) {
                        std::cerr << "[smoothxg] error! path " << smoothed.get_path_name(path)
        ...
        while ((id = kdq_shift_int(q)) != 0) {
            ...
            if (cur_id == ABPOA_SINK_NODE_ID) {
                ...
                if (cur_id != ABPOA_SRC_NODE_ID) {
                    ...
                    std::string seq = std::string(1, static_cast<char>("ACGTN"[abg->node[cur_id].base]));
                    ...
                    for (i = 0; i < abg->node[cur_id].in_edge_n; ++i) {
                        ...
                        if (pre_id != ABPOA_SRC_NODE_ID) {
                            ...
                            // std::cerr << "cur_id edge: " << (cur_id - 1) << std::endl;
                            // std::cerr << "pre_id edge: " << (pre_id - 1) << std::endl;
