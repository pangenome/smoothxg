...
int main(int argc, char **argv) {
    ...
    args::ValueFlag<uint64_t> _max_poa_length(parser, "N", "maximum sequence length to put into POA, cut sequences over this length [default: 2*poa-length-target = 10000]",
                                              {'q', "poa-length-max"});
    args::ValueFlag<uint64_t> _poa_padding(parser, "N", "length of flanking sequence to pad each end of each sequence with during POA, in effect overlapping and trimming the POA problems [default: poa-length-target / 10]",
                                           {'O', "poa-padding"});
    ...
    if (!_read_consensus_path_names) {
        ...
        const uint64_t poa_padding = _poa_padding ? args::get(_poa_padding) : target_poa_length / 10;
