...
int main(int argc, char **argv) {
    ...
    args::ValueFlag<uint64_t> _max_poa_length(parser, "N", "maximum sequence length to put into POA, cut sequences over this length [default: 2*poa-length-target = 10000]",
                                              {'q', "poa-length-max"});
    args::ValueFlag<float> _poa_padding_ratio(parser, "N", "flanking sequence length ratio (with respect to the longest sequence in the block) to pad each end of each sequence with during POA, in effect overlapping and trimming the POA problems [default: 10]\"",
                                           {'O', "poa-padding-ratio"});
    ...
    if (!_read_consensus_path_names) {
        ...
        const float poa_padding_ratio = _poa_padding_ratio ? args::get(_poa_padding_ratio) : 10.0;
