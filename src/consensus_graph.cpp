...
bool operator<(const link_path_t& a,
    ...
    return std::tie(a_0, a_1, a.length, a.jump_length, a.begin, a.end, as_integer(a.path))
        < std::tie(b_0, b_1, b.length, b.jump_length, b.begin, b.end, as_integer(b.path));
}
                ...
                [&](const std::vector<link_path_t> &links) {
                    ...
                    std::sort(unique_links.begin(),
                              ...
                              [&](const link_path_t& a,
                                  ...
                                  auto& b_0 = as_integer(b.from_cons_path);
                                  auto& b_1 = as_integer(b.to_cons_path);
                                  return std::tie(hash_counts[a.hash], a_0, a_1, a.begin, a.end, a.length, a.jump_length, as_integer(a.path))
                                      > std::tie(hash_counts[b.hash], b_0, b_1, b.begin, b.end, b.length, b.jump_length, as_integer(b.path));
...
#pragma omp parallel for schedule(dynamic,1)
    ...
    // FIXME could this be run in parallel? --> create the path in an extra for
    {
        ...
        for (auto& link : consensus_links) {
            ...
            if (link.length > 0) {
                stringstream s;
                s << "Link_" << *link.from_cons_name << "_" << *link.to_cons_name << "_" << link.rank;
                ...
                for (step_handle_t step = (smoothed.has_next_step(link.begin)
                ...
                if (link.begin != link.end) {
                    link_steps(link.begin, link.end);
                }
            }
        }
    }
    ...
    */
    ...
    consensus->apply_ordering(odgi::algorithms::groom(*consensus, true));
    odgi::algorithms::unchop(*consensus, thread_count, true);
